...
TEST_CASE("structopt can parse array optional argument" * test_suite("array_optional")) {
  {
    auto arguments = structopt::parse<ArrayOptionalArgument>(std::vector<std::string>{"./main", "--indices", "1", "2", "3"});
    REQUIRE(arguments.indices == std::array<int, 3>{1, 2, 3});
  }
  {
    auto arguments = structopt::parse<ArrayOptionalArgument>(std::vector<std::string>{"./main", "-i", "4", "5", "6"});
    REQUIRE(arguments.indices == std::array<int, 3>{4, 5, 6});
  }
  {
    auto arguments = structopt::parse<ArrayOptionalArgument>(std::vector<std::string>{"./main"});
